...
function addComponent(holder, mplayer, component) {
    ...
    var constructor = window.Ayamel.controls[component];
...
class ControlBar {
    constructor(args) {
        ...
        control_timeout = 0,
        controlLists = args.components || { left: ["play", "volume", "captions", "annotations"],
        right: ["rate", "fullScreen", "sideToggle", "timeCode"] },
        components = {}, progressBar, timeCode, currentTime = 0,
        duration = 0, element = document.createElement('div');
        ...
        element.appendChild(window.Ayamel.utils.parseHTML(wtemplate));
    ...
    resize() {
        var key, component, el = this.element, left = el.querySelector(".left"), right = el.querySelector(".right"), scale = el.clientWidth / (left.clientWidth + right.clientWidth + 2);
