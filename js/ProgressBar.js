...
class ProgressBar {
	constructor(args) {
		...
		min = +args.min || 0, scale = 100 / (max - min),
		...
		function pxToValue(px) {
			var pmax = parseInt(global.getComputedStyle(element, null).getPropertyValue('width'), 10), pc = Math.min(Math.max(px, 0), pmax) / pmax;
		...
		document.body.addEventListener(isMobile ? "touchmove" : "mousemove", function (event) {
			if (!moving) {
				return;
			}
			lastX = event.pageX;
		...
		document.body.addEventListener(isMobile ? "touchend" : "mouseup", function (event) {
			if (!moving) {
				return;
			}
			moving = false;
			value = pxToValue((isMobile ? lastX : event.pageX) - left);
			level.style.width = (value - min) * scale + "%";
