...
function textMouseup(e){
	...
	if(!rng.intersectsNode(this.contentElement)){ return; }
...
class Text {
	constructor(params) {
		...
		var d_el, c_el, raw = params.content, processor = (typeof params.processor === 'function')
			? params.processor
			: function (x) {
				var el;
				if (typeof x === 'string') {
		...
		Object.defineProperties(this, {
			content: {
				set: (c_el === d_el) ?
					function (t) {
						d_el.innerHTML = "";
						d_el.appendChild(processor(t));
						return raw = t;
					} :
					function (t) {
						var w, nn = processor(t);
						if (nn instanceof DocumentFragment) {
	...
	display(p) {
		...
		if (dp !== this.displayElement.parentNode) {
			...
			this.displayed = true;
			this.displayElement.dispatchEvent(new CustomEvent("display", { bubbles: true, cancelable: true }));
		}
	}
	hide() {
		if (this.displayed) {
			...
			this.displayed = false;
			this.displayElement.dispatchEvent(new CustomEvent("hide", { bubbles: true, cancelable: true }));
		}
	}
	addEventListener(name, handler, capture) {
		this.displayElement.addEventListener(name, handler, capture);
	}
	removeEventListener(name, handler, capture) {
		this.displayElement.removeEventListener(name, handler, capture);
	}
