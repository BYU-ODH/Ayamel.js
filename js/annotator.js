...
class Matcher {
	...
	test(s) { return this.regex.test(s); }
	exec(s) { return this.regex.exec(s); }
}
...
class Annotator {
	...
	addSet(annset) {
		if (this.annotations.indexOf(annset) > -1) {
			return false;
		}
	...
	addEventListener(event, cb, capture) {
		this.element.addEventListener(event, cb, !!capture);
	}
	removeEventListener(event, cb, capture) {
	...
	static AnnSet(label, language, glosses) {
		var mode = "disabled";
		this.element = document.createElement('div');
		this.label = "" + label;
	...
	static loadFor(resource, config) {
		var annotator = new Annotator(config);
		resource.getAnnotations((config.whitelist instanceof Array) ?
			...
			(config.blacklist instanceof Array) ?
				...
				null).then(function (rlist) {
					rlist.map(function (annres) {
						return Ayamel.utils.HTTP({ url: annres.content.files[0].downloadUri })
							.then(function (manifest) {
								return new Ayamel.Annotator.AnnSet(annres.title, annres.languages.iso639_3[0], JSON.parse(manifest));
