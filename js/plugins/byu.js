...
function supportsFile(file){
    return file.streamUri && file.streamUri.substr(0, 8) === 'byu://';
}

function getSignedUrl(args){
    if(!window.BYUEndpoint){
        throw new Error("No Endpoint Configured for BYU Secure plugin");
    }
    ...
    return new Promise(function(resolve, reject){
        ...
        data.append("url", file.streamUri.replace(/^byu:/,"https:"));
        xhr.open("POST", window.BYUEndpoint,true);
...
byuPlayer.prototype.play = function(){
    this.video.play();
};

byuPlayer.prototype.pause = function(){
    this.video.pause();
};

byuPlayer.prototype.enterFullScreen = function(availableHeight){
    this.normalHeight = this.element.clientHeight;
    this.element.style.height = availableHeight + 'px';
};

byuPlayer.prototype.exitFullScreen = function(){
    this.element.style.height = this.normalHeight + 'px';
};

byuPlayer.prototype.addEventListener = function(name, handler, capture){
    this.element.addEventListener(name, handler, !!capture);
};

byuPlayer.prototype.removeEventListener = function(name, handler, capture){
    this.element.removeEventListener(name, handler, !!capture);
};

byuPlayer.prototype.features = {
    desktop: {
        captions: true,
        annotations: true,
        fullScreen: true,
...
export default {
    register: function(ayamel) {
        ayamel.mediaPlugins.video.byu = plugin;
    }
};
